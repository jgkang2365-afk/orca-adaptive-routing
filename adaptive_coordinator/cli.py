...
from .models import InteractionMode, RunMetadata, RunRequest
...
def parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--delegated-by-parent", action="store_true",
                     help="record that an Orca Parent delegated this run")
    run.add_argument("--preapproved", action="store_true",
                     help="record preapproval for normal in-scope operations")
    run.add_argument("--interaction-mode", choices=[mode.value for mode in InteractionMode])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "route":
        ...
        return 0
    if args.command == "run":
        interaction_mode = args.interaction_mode or (
            InteractionMode.NO_INTERVENTION.value if args.preapproved else InteractionMode.STANDARD.value
        )
        metadata = RunMetadata(
            delegated_by_parent=args.delegated_by_parent,
        ...
        result = ProductionRunner(router=router, timeout_ms=args.timeout_ms).run(
            RunRequest(args.task, str(args.workspace), metadata),
        )

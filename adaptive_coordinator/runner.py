...
class ProductionRunner:
    ...
    def _phase_spec(task: str, route: Route, gate: LogicalGateState | None = None,
        ...
        return (f"User task:\n{task}\n\nAssigned logical gate: {gate.logical_gate_id if gate else route.phase.value}; "
                ...
                "variable). Before lifecycle delivery, prepare a gzip-compressed base64url representation of those "
                "exact JSON bytes in memory. Do not create a temporary file or write anywhere to prepare it: READ-ONLY "
                "workers cannot write to /tmp or the workspace. A shell pipeline using printf, gzip -n -c, base64, and "
                "tr may populate another shell variable. The encoded GZ64 payload must be at most 512 characters so "
                "Orca terminal output cannot fold it. If necessary, shorten optional prose and evidence descriptions, "
                "but never shorten or omit exact paths, identifiers, or required phase keys. If the required result still "
                "cannot fit, replace it with a concise failed result using failure_class_hint "
                "INSUFFICIENT_SUCCESS_EVIDENCE; never report a truncated success. Keep the exact three-sentence summary "
                "in a separate variable and set worker_done outcome consistently with the result status. "

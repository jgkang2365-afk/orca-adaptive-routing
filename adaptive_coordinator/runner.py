...
class ProductionRunner:
    ...
    def _phase_spec(task: str, route: Route, gate: LogicalGateState | None = None,
        ...
        return (f"User task:\n{task}\n\nAssigned logical gate: {gate.logical_gate_id if gate else route.phase.value}; "
                ...
                "findings to the Coordinator. Include only the required result fields and concise evidence values. The "
                "complete compact JSON must be at most 768 UTF-8 bytes; shorten values without dropping required keys "
                "if necessary so terminal evidence is never folded or elided by the TUI. "
